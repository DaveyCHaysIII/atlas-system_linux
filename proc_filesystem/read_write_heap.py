...
def search_and_replace_in_heap(pid, heap_start, heap_end, search_string, replace_string):
    """finds and replaces the string in heap memory"""

    mem_file = f"/proc/{pid}/mem"

    if len(search_string) != len(replace_string):
        print("Error: search_string and replace_string must be of the same length.")
        sys.exit(1)
    ...
    try:
        with open(mem_file, "rb+") as mem:
            ...
            print(f"Replaced '{search_string}' with '{replace_string}' at offset {offset:#x}")
...
if __name__ == "__main__":
    ...
    search_and_replace_in_heap(pid, heap_start, heap_end, search_string, replace_string)